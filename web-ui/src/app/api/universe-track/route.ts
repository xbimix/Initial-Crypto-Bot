...
export async function POST(req: Request) {
  ...
  try {
    const response = await fetch(`${BACKEND}/universe-track`, {
      method: "POST",
    ...
    if (shouldUseLocalFallback(response.status)) {
      const fallback = await updateUniverseTrackLocal(body);
      return NextResponse.json(fallback);
    }

    return NextResponse.json(
      {
        error: text.trim() ? text : `Universe track update failed (${response.status})`,
      },
